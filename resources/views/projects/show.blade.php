...
<div class="px-4 sm:px-6 py-8 sm:py-12 md:py-20 relative">
    <div class="max-w-7xl mx-auto">
        ...
        <nav class="flex items-center gap-2 text-xs font-mono text-slate-500 mb-6 sm:mb-8 overflow-x-auto py-1">
            <a href="{{ route('home') }}" class="hover:text-blue-600 transition-colors shrink-0">Beranda</a>
            <span>/</span>
            <a href="{{ route('projects.index') }}" class="hover:text-blue-600 transition-colors shrink-0">Lab &amp; Proyek</a>
            <span>/</span>
            <span class="text-slate-800 truncate max-w-[150px] sm:max-w-xs">{{ $project->title }}</span>
        </nav>
            ...
            <div class="lg:col-span-8 space-y-6 sm:space-y-10">
                ...
                <div class="space-y-3 sm:space-y-4">
                    <div class="flex items-center gap-3">
                        <span class="px-3 py-1 rounded-full bg-blue-50 border border-blue-200 text-xs font-mono font-medium text-blue-700">
                            {{ $project->category }}
                        </span>
                    ...
                    <h1 class="text-2xl sm:text-4xl lg:text-5xl font-bold text-slate-900 tracking-tight leading-[1.15] text-balance">
                        {{ $project->title }}
                    </h1>
                </div>

                <!-- Topology Preview Frame -->
                <div class="relative rounded-2xl sm:rounded-3xl overflow-hidden border border-slate-200/80 bg-slate-50 p-4 sm:p-8 flex items-center justify-center shadow-md">
                    <div class="absolute inset-0 bg-gradient-to-tr from-blue-600/5 via-teal-500/5 to-transparent pointer-events-none"></div>
                ...
                <div class="p-5 sm:p-8 rounded-2xl sm:rounded-3xl glass-panel space-y-5 sm:space-y-6">
                    ...
                    <div class="text-slate-700 leading-relaxed text-sm sm:text-base lg:text-lg space-y-4 whitespace-pre-line text-balance">
            ...
            <div class="lg:col-span-4 space-y-6">
                ...
                <div class="p-5 sm:p-8 rounded-2xl sm:rounded-3xl glass-panel space-y-5 sm:space-y-6">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-4">
                        <div class="text-xs font-mono font-bold text-blue-600 uppercase tracking-wider">Spesifikasi Lab</div>
                    ...
                    <dl class="space-y-4 font-mono text-xs">
                        <div>
                            ...
                            <dd class="text-slate-900 font-semibold text-sm">{{ $project->category }}</dd>
                        </div>
                        ...
                        <div>
                            ...
                            <dd class="flex flex-wrap gap-1.5">
                                @foreach($project->tools_list as $tool)
                                    <span class="px-3 py-1 rounded-full glass-pill text-xs text-slate-700">
                                        {{ $tool }}
                                    </span>
                                @endforeach
                            </dd>
                        ...
                        <div>
                            ...
                            <dd class="text-emerald-600 font-semibold flex items-center gap-1.5">
                        ...
                        <div>
                            ...
                            <dd class="text-slate-900 font-semibold">I Made Yuda Pramana</dd>
                        </div>
                    </dl>
                ...
                @if($relatedProjects->count() > 0)
                    <div class="p-5 sm:p-8 rounded-2xl sm:rounded-3xl glass-panel space-y-4">
                        <div class="text-xs font-mono font-bold text-blue-600 uppercase tracking-wider border-b border-slate-100 pb-3">
                            Lab Terkait
                        </div>

                        <div class="space-y-3">
                            @foreach($relatedProjects as $rel)
                                <a href="{{ route('projects.show', $rel->slug) }}" class="block p-4 rounded-2xl glass-panel-interactive hover:border-blue-300 transition-all group">
                                    <div class="text-[10px] font-mono text-slate-400 mb-1">{{ $rel->category }}</div>
                                    <div class="text-sm font-semibold text-slate-900 group-hover:text-blue-600 transition-colors line-clamp-2">
                                        {{ $rel->title }}
                                    </div>
                                </a>
                            @endforeach
